...
// git 실행 파일을 쓰기 가능한 임의 PATH로 탐색하지 않도록, 현재 프로세스가 물려받은
// PATH를 명시적으로 고정해서 넘긴다 (SonarCloud javascript:S4036).
const GIT_ENV = { ...process.env, PATH: process.env.PATH };
...
export function getCommits(cwd, sinceIso) {
  ...
  let output;
  try {
    output = execFileSync('git', args, { cwd, env: GIT_ENV, maxBuffer: 1024 * 1024 * 50 }).toString();
  } catch {
    return [];
  }
