...
각 상호작용마다 다음을 0~100점으로 평가하세요:
- context_score: 문제 해결에 필요한 맥락(코드, 에러 메시지, 제약조건 등)을 충분히 제공했는가
- clarity_score: 요구사항과 목표가 명확한가, 모호하지 않은가
...
reasoning에는 왜 이 레벨로 판단했는지, 어떤 행동 패턴이 근거였는지 구체적으로 쓰세요.
next_level_conditions에는 다음 단계로 가기 위해 무엇을 바꿔야 하는지 구체적으로 쓰세요."""
